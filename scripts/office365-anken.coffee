# Description:
#   Office365の案件管理ワークシートのデータを参照します。
# 
# Dependencies:
#   None
#
...
#   O365_CLIENT_ID 案件ボット用のOffice365アプリClientId
#
# Commands:
...
findAnken = (msg, field, value, cb) ->
  ...
  fs.exists ankenPath, (exists) ->
    ...
    office.onedrive.getClient (client) ->
      client.getFileByName ANKEN_WORKSHEET_NAME, (err, file) ->
        if err?
          console.log err
        else
          if exists and file? and file.eTag == fs.readFileSync(ankenPath + ".etag", "utf8")
            ...
            client.downloadFile file.id, ankenPath, (err) ->
              if err?
                console.log err
              else
